...
def All_Metrics(pred, true, mask1, mask2):
    """Compute all metrics including MAE, RMSE, RRSE, IC, and RIC"""
    ...
    if type(pred) == torch.Tensor:
        ...
        # Flatten tensors for correlation calculations
        pred_flat = pred.reshape(-1)
        true_flat = true.reshape(-1)

        # Calculate IC (Pearson correlation)
        ic = pearson_correlation(pred_flat, true_flat)

        # Calculate RIC (Rank Information Coefficient / Spearman correlation)
        ric = rank_information_coefficient(pred_flat, true_flat)
    else:
        raise TypeError

    return mae, rmse, rrse, ic, ric
...
def rank_tensor(x):
    ...
    ranks[sorted_indices] = torch.arange(
        1, len(x) + 1, dtype=torch.float, device=sorted_indices.device
    )
