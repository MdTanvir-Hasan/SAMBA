...
class MinMaxNorm01:
    ...
    def fit(self, x):
        # Store min/max as tensors if x is a tensor, otherwise as scalars
        if torch.is_tensor(x):
            self.min = x.min().detach().cpu()
            self.max = x.max().detach().cpu()
        else:
            self.min = x.min()
            self.max = x.max()
    ...
    def inverse_transform(self, x):
        # Ensure min/max are on the same device as x if x is a tensor
        if torch.is_tensor(x):
            min_val = torch.tensor(self.min, dtype=x.dtype, device=x.device)
            max_val = torch.tensor(self.max, dtype=x.dtype, device=x.device)
...
def data_loader(X, Y, batch_size, shuffle=True, drop_last=True):
    ...
    # Data is already on the correct device from prepare_data
    # Just wrap in TensorDataset and DataLoader
...
def prepare_data(csv_file, window=5, predict=1, test_ratio=0.15, val_ratio=0.05):
    ...
    # Basic preprocessing
    name = X["Name"].iloc[0]  # Use .iloc[0] instead of [0] to avoid FutureWarning
    del X["Name"]
    cols = X.columns
    ...
    X_test = XX[:test_len, :, :].float().to(device)
    Y_test = YY[:test_len, :, :].float().to(device)

    X_train = XX[test_len : test_len + train_len, :, :].float().to(device)
    Y_train = YY[test_len : test_len + train_len, :, :].float().to(device)

    X_val = XX[-val_len:, :, :].float().to(device)
    Y_val = YY[-val_len:, :, :].float().to(device)
