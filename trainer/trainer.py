...
class Trainer:
    ...
    def val_epoch(self, epoch, val_dataloader):
        ...
        with torch.no_grad():
            for batch_idx, (data, target) in enumerate(val_dataloader):
                # Data is already on the correct device from data loader
                label = target
    ...
    def train(self):
        ...
        y_pred, y_true = self.test(self.model, self.args, self.test_loader, self.logger)

        # Calculate final test metrics (now includes IC and RIC)
        test_mae, test_rmse, _, test_ic, test_ric = All_Metrics(
            y_pred, y_true, self.args.get("mae_thresh"), self.args.get("mape_thresh")
        )
        ...
        metrics = {
            ...
            "train_loss_history": [float(x) for x in train_loss_list],
            "val_loss_history": [float(x) for x in val_loss_list],
            "total_epochs": len(train_loss_list),
            "converged": train_epoch_loss < 1e6,
            "early_stopped": not_improved_count
            == self.args.get("early_stop_patience", 0),
        }
    ...
    def test(model, args, data_loader, logger, path=None):
        ...
        mae, rmse, _, ic, ric = All_Metrics(
            y_pred, y_true, args.get("mae_thresh"), args.get("mape_thresh")
        )
        logger.info(
            "Average Horizon, MAE: {:.4f}, RMSE: {:.4f}, IC: {:.4f}, RIC: {:.4f}".format(
                mae, rmse, ic, ric
            )
        )
