...
def create_per_run_metrics_plot():
    ...
    print(f"📁 Found {len(exp_dirs)} experiment folder(s) to process")

    # Collect data from all runs for each experiment folder
    for exp_dir in exp_dirs:
        ...
        print(f"📊 Processing {exp_dir.name}: Found {len(run_dirs)} runs")
    ...
    print(
        f"✅ Collected data from {len(df_runs)} runs across {len(df_runs['Dataset'].unique())} datasets"
    )
    ...
    metrics = ["RMSE", "IC", "RIC"]
    datasets = sorted(df_runs["Dataset"].unique())
    colors = {dataset: f"C{i}" for i, dataset in enumerate(datasets)}
